...
move_uploaded_file($file_tmp,$path);  
  if(!empty($_POST['tag'])){
    ...
    if(!empty($_POST)) {
      // var_dump($_POST);
  		$sql1 = "UPDATE `new`,`category` SET `title`='$title',`id_category`='$id_category',`content`='$content',`description`='$description',`start_date` = '$date_time[0]',`end_date` = '$date_time[1]',`avatar` = '$path' WHERE new.id= {$id} AND new.id_category = category.id";
